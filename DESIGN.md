...
themes:
  brand-light:
    class: null
    description: Clay cream canvas with saturated brand accents (default)
  brand-dark:
    class: dark
    description: Clay dark teal canvas; inverted primaries; brand chart colors retained
    colors:
      canvas: "#0a1a1a"
      surface-soft: "#0a1a1a"
      surface-card: "#1a2a2a"
      surface-strong: "#1a2a2a"
      surface-dark: "#0a1a1a"
      surface-dark-elevated: "#1a2a2a"
      ink: "#ffffff"
      body: "#ffffff"
      ...
      muted-soft: "#a0a0a0"
      hairline: "#ffffff1f"
      hairline-soft: "#ffffff1f"
      primary: "#ffffff"
      primary-active: "#f5f5f5"
      primary-disabled: "#1a2a2a"
      on-primary: "#0a0a0a"
      on-dark: "#ffffff"
      on-dark-soft: "#a0a0a0"
  wireframe:
    class: wireframe
    description: shadcn neutral base — grayscale only; use for layout and component structure without brand color
    colors:
      primary: "#171717"
      ...
      surface-dark: "#171717"
      surface-dark-elevated: "#404040"
      on-primary: "#fafafa"
      on-dark: "#fafafa"
      on-dark-soft: "#a3a3a3"
      brand-pink: "#525252"
      brand-teal: "#404040"
      brand-lavender: "#737373"
      brand-peach: "#a3a3a3"
      brand-ochre: "#d4d4d4"
      brand-mint: "#a3a3a3"
      brand-coral: "#737373"
    ...
    chart:
      ...
      chart-5: "#d4d4d4"
    component-mapping:
      feature-card-pink: feature-card-tone-1
      feature-card-teal: feature-card-tone-2
      feature-card-lavender: feature-card-tone-3
      feature-card-peach: feature-card-tone-4
      feature-card-ochre: feature-card-tone-5
      feature-card-cream: feature-card-neutral
...
Three appearance modes ship with this spec. Top-level `{colors.*}` tokens are **brand-light** (Clay). Override via CSS class on `<html>`:

| Theme | HTML class | Purpose |
|---|---|---|
| **brand-light** | _(none)_ | Default Clay marketing palette — cream canvas, saturated feature cards |
| **brand-dark** | `dark` | Clay dark mode — teal-tinted surfaces, inverted primary |
| **wireframe** | `wireframe` | shadcn **neutral** grayscale — white canvas, gray accent slots, no brand color |

### Wireframe mode

Use `{themes.wireframe}` when exploring layout, hierarchy, and component states without brand distraction:

- Canvas is pure white (`{themes.wireframe.colors.canvas}` — #ffffff), not cream.
- Brand accent slots (`brand-pink` … `brand-ochre`) map to neutral steps (#404040–#d4d4d4) so `{component.feature-card-*}` keys stay stable but render as gray tones.
- Chart / stat surfaces use `{themes.wireframe.chart.*}` — five neutral steps, not pink/teal/lavender.
- Semantic success/warning/error stay colored for state clarity.
- In code: add `wireframe` to `<html className="wireframe">` or `next-themes` with `themes={["light", "dark", "wireframe"]}`. Do not combine `wireframe` with `dark` unless you define a separate dark-wireframe palette.
...
8. For structure-only work, switch to `{themes.wireframe}` (`wireframe` class) before iterating on layout.
